...
public class PromotionsStepDefinition extends BaseStepDefinition {
    ...
    public void init() {
       super.init();
		...
		public void refuse_the_coockies() throws Throwable {
		...
		@And("^User on \"([^\"]*)\" page$")
		public void user_on_something_page(String pagename) throws Throwable {
			page = pagename;
		    String expectedUrl = BasePage.PAGE_URLS.get(pagename);
			String actualUrl = BasePage.driver.getCurrentUrl();
			Assert.assertEquals(expectedUrl,actualUrl);
		...
		public void user_add_the_product_to_panier() throws Throwable {
			//getPage(BasePage.driver). clickajoutPanierDir();
			String messageActual = getPage(BasePage.driver).getMessageAjoutPanier();
			//System.out.println(messageActual);
		    Assert.assertTrue(messageActual.contains("Produit"));
